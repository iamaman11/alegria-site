use contracts::generated::alegria::temporal::v1::{
    CrawlSourcesInputPayload, SeoSiteBuildInputPayload, SeoVerifiedFactSupportState,
...
pub(crate) fn register(opts: &mut WorkerOptions) {
    opts.register_workflow::<ExpertSemanticSliceWorkflow>();
}
...
#[workflow_methods]
impl ExpertSemanticSliceWorkflow {
    #[run]
    async fn run(ctx: &mut WorkflowContext<Self>) -> WorkflowResult<String> {
        metrics::global()
            .workflow_starts_total
            .with_label_values(&["ExpertSemanticSliceWorkflow"])
            .inc();
        ...
        {
            ctx.state_mut(|s| s.phase = "done:semantic_slice_skipped".to_string());
            metrics::global()
                .workflow_completions_total
                .with_label_values(&["ExpertSemanticSliceWorkflow"])
                .inc();
            return Ok(format!(
                "expert_semantic_slice_skipped run_id={} support_bundle={} page_mode={:?}",
                run_id,
                support_bundle.len(),
                page_utility.page_mode
            ));
        }
        ...
        let contradiction_facts = procedural
            .rules
            .iter()
        ...
        let hitl = ctx
            .start_activity(
                ...
                seo_steps::hitl_decision_step::HitlDecisionInput {
                    run_id: run_id.clone(),
                    step_name: "expert_semantic_slice".to_string(),
        ...
        ctx.state_mut(|s| s.phase = "done:expert_semantic_slice".to_string());
        metrics::global()
            .workflow_completions_total
            .with_label_values(&["ExpertSemanticSliceWorkflow"])
            .inc();
        ...
        Ok(format!(
            "expert_semantic_slice_ok run_id={} support_bundle={} section_id={} primary_layer={} mentions={} mapped={} procedural_rules={} operational_entities={} editorial_topics={} triples={} completeness_score={:.2} conflicts={} hitl={}",
            run_id,
            support_bundle.len(),
            section.section_id,
            layer.primary_layer,
            ...
            operational.entities.len(),
            editorial.topics.len(),
            triples.triples.len(),
