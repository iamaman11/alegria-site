...
pub fn verify_fact_json(fact_key: &str, candidates_json: &str, registry_json: &str) -> String {
    ...
    serde_json::to_string(&verify_fact_typed(fact_key, &candidates, &registry)).unwrap_or_else(
        |_| "{\"resolution\":\"hitl_required\",\"value\":null,\"diagnostics\":[]}".to_string(),
    )
}
...
pub fn verify_numeric_rule_json(rule_key: &str, candidates_json: &str, field_name: &str) -> String {
    ...
    serde_json::to_string(&verify_numeric_rule_typed(
        rule_key,
        &candidates,
        field_name,
    ))
