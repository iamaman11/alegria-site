...
use seo_ports::{
    ...
    GraphReasoningPort, HitlQueuePort, OrganicSerpResponse, OrganicSerpResult, PlanningRepository,
    ProjectionBarrierStatus, ProjectionStatusRepository, PublishArtifactRepository,
    RebuildDependencyEvidence, RebuildRepository, SectionTemplateRepository, SemanticDemandCluster,
    SemanticLinkCandidate, SemanticLinkSearchPort, SeoBuildInputRepository,
...
use super::{
    dataforseo_serp_adapter, editorial_llm_adapter, neo4rs_adapter, raw_crawl_adapter,
...
async fn ensure_graph_contract_if_required(context_key: &str) -> Result<(), DomainError> {
    seo_steps::read_neo4j_context::read_neo4j_context(context_key)
        .await
        .map_err(|err| DomainError::InfraUnavailable {
            message: format!("graph contract probe failed for `{context_key}`: {err}"),
        })
}
