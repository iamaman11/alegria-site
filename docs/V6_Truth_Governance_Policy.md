...
- the accepted baseline now includes explicit fixture proofs for mirror non-independence, weak-source corroboration rejection, single authoritative override, override denial for non-authoritative source classes, and stale primary-authority override blocking;
...
4. `entity_span_detection_step` via deterministic token and phrase parsing
